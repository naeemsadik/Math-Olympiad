...
function buildPayload(form: Test) {
  ...
  return {
    ...
    random_question_count: form.testType === "diagnostic" ? form.randomQuestionCount ?? 10 : null,
    advanced_threshold: form.testType === "diagnostic" ? form.advancedThreshold ?? 50 : null,
    expert_threshold: form.testType === "diagnostic" ? form.expertThreshold ?? 80 : null,
...
export default function AdminTestsPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const testsList = Array.isArray(testsRes) ? testsRes : (testsRes as { data?: Test[] }).data ?? [];
      const questionsList = Array.isArray(questionsRes) ? questionsRes : (questionsRes as { data?: import("@/types").Question[] }).data ?? [];
      setTests(testsList.map((t) => normalizeTest(t)));
      setQuestions(questionsList);
    } catch (e) {
      const msg = e instanceof ApiError ? e.message : "Failed to load tests.";
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  const filtered = useMemo(() => {
    ...
    return tests.filter((test) =>
      ...
      (test.topicId ?? "").toLowerCase().includes(needle)
    );
  }, [tests, search]);

  const selectedQuestions = useMemo(
    () => (form.questionIds ?? []).map((id) => questions.find((question) => question.id === String(id) || question.id === id)).filter(Boolean) as import("@/types").Question[],
    [form.questionIds, questions]
  );

  const availableQuestions = useMemo(() => {
    const needle = questionSearch.toLowerCase();
    const selected = new Set((form.questionIds ?? []).map((id) => String(id)));
    return questions
  ...
  const removeQuestion = (id: string) => {
    setForm((prev) => {
      const questionIds = (prev.questionIds ?? []).filter((questionId) => String(questionId) !== String(id));
      return normalizeTest({ ...prev, questionIds, questionCount: questionIds.length });
    });
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editId) {
        await api.admin.tests.update(editId, payload as unknown as Record<string, unknown>);
      } else {
        await api.admin.tests.create(payload as unknown as Record<string, unknown>);
      }
      await load();
      setShowForm(false);
    } catch (e) {
      const msg = e instanceof ApiError ? e.message : "Failed to save test.";
      setError(msg);
    } finally {
      setSaving(false);
    }
  ...
  const doDelete = async () => {
    if (!deleteId) return;
    try {
      await api.admin.tests.remove(deleteId);
      setTests((prev) => prev.filter((t) => t.id !== deleteId));
      setDeleteId(null);
    } catch (e) {
      const msg = e instanceof ApiError ? e.message : "Failed to delete test.";
      setError(msg);
    }
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="glass rounded-2xl p-6 border border-[#d97706]/30 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-2 max-w-sm">
            {(["diagnostic", "practice"] as TestType[]).map((type) => (
              <button
                key={type}
                onClick={() => setForm((prev) => normalizeTest({ ...prev, testType: type }))}
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4 space-y-3">
              ...
              <div className="max-h-72 overflow-y-auto space-y-2 pr-1">
                {availableQuestions.map((question) => (
                  <button key={question.id} onClick={() => addQuestion(String(question.id))} className="w-full text-left rounded-xl border border-slate-100 bg-slate-50 p-3 hover:border-[#d97706]/30 hover:bg-[#d97706]/5 transition-colors">
              ...
              <div className="max-h-80 overflow-y-auto space-y-2 pr-1">
                {selectedQuestions.map((question) => question && (
                  <div key={question.id} className="rounded-xl border border-slate-100 bg-slate-50 p-3 flex items-start gap-2">
                    <button onClick={() => removeQuestion(String(question.id))} className="mt-0.5 text-slate-400 hover:text-red-500">
                      <MinusCircle size={15} />
                    </button>
                    <div className="min-w-0">
                      <p className="text-sm text-slate-800 line-clamp-2">{question.prompt?.kind === "image" ? "Image prompt" : stripMathSyntax(question.prompt?.value ?? question.content)}</p>
          ...
          <div className="flex gap-3 justify-end">
            ...
            <button onClick={save} disabled={saving} className="flex items-center gap-2 gradient-orange text-white text-sm font-semibold px-5 py-2 rounded-xl hover:scale-105 transition-all disabled:opacity-50">
              <Check size={15} /> {saving ? "Saving…" : editId ? "Save Changes" : "Create Test"}
            </button>
          </div>
        </div>
      )}
      ...
      <div className="glass rounded-2xl overflow-hidden">
        {loading ? (
          <p className="text-center text-slate-400 text-sm py-10">Loading tests…</p>
        ) : (
