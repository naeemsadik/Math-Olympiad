...
import { topics as mockTopics } from "@/lib/mock/topics";
import { api } from "@/lib/api";
import { BookOpen, ArrowRight, Lock } from "lucide-react";
import { useAuthStore } from "@/store/authStore";
import type { Tier, Topic } from "@/types";
...
export default function TopicsPage() {
  ...
  useEffect(() => {
    ...
    api.public
      .getTopics()
      .then((res) => {
        if (!cancelled && Array.isArray(res) && res.length) setTopics(res);
      })
    ...
    <div className="space-y-8">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filteredTopics.map((topic) => (
          <Link
            key={topic.id}
            href={`/student/topics/${topic.slug}`}
