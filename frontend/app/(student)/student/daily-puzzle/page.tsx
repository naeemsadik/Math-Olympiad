...
export default function DailyPuzzlePage() {
  ...
  const userTier = (user?.tier ?? "Beginner") as Tier;
  const fallback = puzzlesByTier[userTier];
  const [puzzle, setPuzzle] = useState(fallback);
  ...
  useEffect(() => {
    ...
    api.puzzles
      ...
      .then((data) => {
        ...
        const todayPuzzle = data as { id: string; date: string; content: string; topic: string; difficulty: string; tier: string; streakCount: number };
        // Merge API fields into our local DailyPuzzle shape (keeps mock previousPuzzles + topSolvers).
        setPuzzle((prev) => ({
          ...prev,
          id: todayPuzzle.id,
          date: todayPuzzle.date ?? prev.date,
          content: todayPuzzle.content ?? prev.content,
          topic: todayPuzzle.topic ?? prev.topic,
          tier: (todayPuzzle.tier as Tier) ?? prev.tier,
          difficulty: (todayPuzzle.difficulty as Difficulty) ?? prev.difficulty,
          streakCount: todayPuzzle.streakCount ?? prev.streakCount,
        }));
      })
      .catch(() => {/* keep fallback */});
    return () => {
