...
import { leaderboard as mockLeaderboard } from "@/lib/mock/dashboard";
...
const allTimeData = mockLeaderboard.map((e, i) => ({ ...e, rating: e.rating + 1200 - i * 30 }));
...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    api.public
      .getLeaderboard({ period: tab, tier: tierFilter === "All" ? undefined : tierFilter, limit: 50 })
      .then((res) => {
        if (!cancelled && Array.isArray(res)) setEntries(res);
      })
      .catch(() => {
        if (!cancelled) setEntries(mockLeaderboard);
      });
    return () => {
      cancelled = true;
    };
  ...
  const fallback = tab === "monthly" ? mockLeaderboard : allTimeData;
  const rawData = entries ?? fallback;
