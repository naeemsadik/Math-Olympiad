...
export default function VerifyPage() {
  ...
  const handleSearch = async () => {
    ...
    // Try the real API first. Fall back to local mock matching if API is not configured or fails.
    try {
      const data = await api.public.verifyCertificate(q);
      const list = (Array.isArray(data) ? data : [data]) as unknown[];
      const mapped: Certificate[] = list
        .filter((c): c is Record<string, unknown> => !!c && typeof c === "object")
        .map((c) => {
          const obj = c as Record<string, unknown>;
          const status = String(obj.status ?? "active") === "revoked" ? "revoked" : "valid";
          return {
            id: String(obj.id ?? obj.certificate_id ?? ""),
            studentName: String(obj.studentName ?? obj.recipient_name ?? ""),
            studentId: String(obj.studentId ?? obj.student_id_str ?? ""),
            dept: String(obj.dept ?? ""),
            institute: String(obj.institute ?? ""),
            achievement: String(obj.achievement ?? obj.title ?? ""),
            event: String(obj.event ?? ""),
            eventType: String(obj.eventType ?? ""),
            description: String(obj.description ?? ""),
            issuedAt: String(obj.issuedAt ?? obj.issued_at ?? ""),
            tier: String(obj.tier ?? ""),
            issuedBy: String(obj.issuedBy ?? "UIU Centre for Math Olympiad and Research"),
            signatoryName: String(obj.signatoryName ?? obj.signatory_name ?? ""),
            signatoryTitle: String(obj.signatoryTitle ?? obj.signatory_title ?? ""),
            status,
          };
        });
      setResults(mapped);
      ...
      if (err instanceof ApiError && err.status === 404) {
        ...
        // Fallback to mock for offline / dev-without-backend.
        const qLower = q.toLowerCase();
        const found = mockCertificates.filter((c) =>
          searchType === "id"
            ? c.id.toLowerCase() === qLower
            : c.studentId.toLowerCase().includes(qLower)
        );
        setResults(found);
      }
    } finally {
