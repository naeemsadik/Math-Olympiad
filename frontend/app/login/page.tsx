...
export default function LoginPage() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    setSiLoading(true);
    try {
      const user = await login(siEmail.trim(), siPass);
      if (user.role === "ADMIN") {
        router.push("/admin/dashboard");
      } else if (user.placementDone) {
      ...
      if (err instanceof ApiError) {
        if (err.status === 422 || err.status === 401) {
          setSiError(err.errors?.email?.[0] ?? err.message ?? "Invalid credentials.");
        } else if (err.status === 403) {
          setSiError("You don't have access to that account.");
        } else if (err.status === 0) {
          setSiError("Cannot reach the server. Check NEXT_PUBLIC_API_URL and that the backend is running.");
        } else {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      await register({
        ...
        email: suEmail.trim(),
        password: suPass,
        password_confirmation: suConfirm,
      ...
      if (err instanceof ApiError) {
        if (err.status === 422 && err.errors) {
          ...
          setSuError(err.errors[firstField]?.[0] ?? err.message);
        } else if (err.status === 0) {
          setSuError("Cannot reach the server. Check NEXT_PUBLIC_API_URL and that the backend is running.");
        } else {
